...
export function MatchingPanel({ doctorId, doctorName, doctorEmail }: { doctorId: string, doctorName: string, doctorEmail: string }) {
  ...
  useEffect(() => {
    const fetchRecipients = async () => {
      ...
      try {
        ...
        console.log('Found compatible recipients:', sorted);
  ...
  const assignMatch = async (recipient: Recipient) => {
    setLoading(true)
    setError("")
    try {
      ...
      try {
        ...
        console.log('Sent email to donor:', compatibleDonor.email);
        ...
        // Don't throw here, we still want to consider the match successful
      }
    } catch (err: any) {
      console.error('Error in assignMatch:', err);
